...
test("codex plugin manifest exists and is host-neutral", () => {
  ...
  assert.equal(parsed.interface.displayName, "OpenCode");
...
test("codex marketplace entry points at the plugin source", () => {
  ...
  const entry = parsed.plugins.find((plugin) => plugin.name === "opencode");
  assert.ok(entry, "Missing opencode entry");
  assert.equal(entry.source.source, "local");
  assert.equal(entry.source.path, "./plugins/opencode");
